...
import { beatTicks, barTicks, midi, motif, tick } from '../../core/index.js';
...
export function generateDrums(g: GenContext, params: Genome['drums'], rng: Rng): Motif {
  ...
  for (let b = 0; b < nBars; b++) {
    ...
    for (let i = 0; i < g.meter.num; i++) {
      ...
      hit(t, i % 2 === 0 ? KICK : SNARE);
      hit(t, HAT, 58);
      const andT = t + eighth + swingOff;
      if (andT < g.harmony.length) hit(andT, HAT, 48);
    }
    ...
    if (fill) {
      ...
      const count = Math.max(1, Math.round(subdiv * (0.5 + params.fillDensity * 0.5)));
